...
function Formulario() {
  ...
  const onSubmit = (data) => {
    ...
    // Agregar producto al catálogo si no existe
    const productosCatalogo = JSON.parse(localStorage.getItem('productos') || '[]');
    const yaExiste = productosCatalogo.some(p => p.name === data.producto);
    ...
    if (!yaExiste) {
      const nuevoProducto = {
        ...
        category: 'pedido',
        price: 0,
        stock: parseInt(data.cantidad) || 1
      };
      productosCatalogo.push(nuevoProducto);
      localStorage.setItem('productos', JSON.stringify(productosCatalogo));
    }
